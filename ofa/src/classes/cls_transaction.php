	...
	*/
	...
	class cls_transaction extends cls_sql_table 
	{
		...
		{	
			...
			$cpv2       = new cls_cpv2       ( $this->db );
			$cpv3       = new cls_cpv3       ( $this->db );
			...
			foreach ( $arrDetail['header'] as $key) 
			{
				...
				switch (trim(strtolower($key['field']))) 
				{
					...
					case 'attribute2'        :
						$cpv2->set_id ( intval ( $valor ) ) ; 
						break;
					case 'attribute4'        :       
						$cpv3->set_id ( intval ( $valor ) ) ; 
						break;
				}
			}
			...
			echo '</td></tr><tr><td width=150><b>'.$cpv3->caption().'</b></td><td>';
			$cpv3->titulo();
										...
										$estadoInicial = -1 ) 
		{
			...
			if ($this->validarNumeroPedido())
			{
				...
				$this->oInterface->iface_header->set_detail ( 
					$this->oInterface->iface_header->get_id_field() , 
					$header_id
				);

				//genero los registros de lineas

				$this->oInterface->iface_lines->InsertLocal (
					$this->oInterface->iface_header ,
					$customer , $list     , $term       , $salesrep ,
					$products , $pay_term , $order_type , 
					$cpv2     , $cpv3     , $this     
				);
